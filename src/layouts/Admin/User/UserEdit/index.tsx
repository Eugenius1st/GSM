...
const UserRegist = () => {
  ...
  useEffect(() => {
    if (data) {
      const categorizedProsData = data.improvements.reduce(
        (acc: any, curr: any) => {
  ...
  function validateInputs() {
    ...
    if (
      ...
      if (userAge <= 14) {
        alert(
          "14세 이하의 경우, 부모님 중 적어도 한 분의 전화번호를 정확히 입력해주세요."
        );
        return false;
      }
    } else if (
      technicalSkillPros.length < 3 ||
      mentalSkillPros.length < 3 ||
      physicalSkillPros.length < 3
    ) {
      alert("장점을 각각 3개 이상씩 선택하세요");
      return false;
    } else if (
      technicalSkillImpro.length < 3 ||
      mentalSkillImpro.length < 3 ||
      physicalSkillImpro.length < 3
    ) {
  ...
  return (
    <div className="eg-regist-wrapper">
      ...
      <div className="flex justify-end mt-2">
        <WhiteBtn content="취소" func={() => navigate(-1)} />
        <PurpleBtn content="수정" func={patchStudent} />
      </div>
    </div>
  );
};
