...
handleState) => {
  ...
  const [checkTechnicalImporve, setCheckTechnicalImporve] = useState<
    skillType[]
  >([]);
  ...
  function validateInputs() {
    if (
      checkTechnicalPros.length < 3 ||
      checkMentalPros.length < 3 ||
      checkPhysicalPros.length < 3
    ) {
  ...
  return (
    <div>
      ...
      <div className="p-2 mt-6 border border-egGrey-default">
        ...
        <TagCard tagList={technicalSkill} func={setCheckTechnicalPros} />
      </div>
      <div className="p-2 mt-6 border border-egGrey-default">
        <div className="mt-1 mb-2">정신적 능력 선택(3가지 이상)</div>
        <TagCard tagList={mentalSkill} func={setCheckMentalPros} />
      </div>
      <div className="p-2 mt-6 border border-egGrey-default">
        <div className="mt-1 mb-2">신체적 능력 선택(3가지 이상)</div>
        <TagCard tagList={physicalSkill} func={setCheckPhysicalPros} />
      </div>
      ...
      <div className="p-2 mt-6 border border-egGrey-default">
        <div className="mt-1 mb-2">기술적 능력 선택(3가지 이상)</div>
        <TagCard tagList={technicalSkill} func={setCheckTechnicalImporve} />
      </div>
      <div className="p-2 mt-6 border border-egGrey-default">
        <div className="mt-1 mb-2">정신적 능력 선택(3가지 이상)</div>
        <TagCard tagList={mentalSkill} func={setCheckMentalImporve} />
      </div>
      <div className="p-2 mt-6 border border-egGrey-default">
        <div className="mt-1 mb-2">신체적 능력 선택(3가지 이상)</div>
        <TagCard tagList={physicalSkill} func={setCheckPhysicalImporve} />
      </div>
      ...
      <div className="flex justify-end my-8">
        {registStage > 1 && registStage < 5 && (
          <WhiteBtn content="이전" func={handlePreview} />
        )}
        {registStage < 5 && (
          <PurpleBtn
