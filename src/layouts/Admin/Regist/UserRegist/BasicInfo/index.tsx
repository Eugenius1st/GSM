...
const BasicInfo = ({
  ...
  function validateInputs() {
    ...
    if (!idRegex.test(userID)) {
      alert(
        "ID는 5~16자의 영문 소문자, 숫자, 언더바(_)로만 이루어져야 합니다."
      );
      return false;
      ...
      if (userAge <= 19) {
        alert(
          "19세 이하의 경우, 부모님 중 적어도 한 분의 전화번호를 정확히 입력해주세요."
        );
        return false;
      }
    }
  ...
  function handleDuplicate(e: any) {
    ...
    if (!idRegex.test(userID)) {
      alert(
        "ID는 5~16자의 영문 소문자, 숫자가 포함되어야 하고, 언더바(_) 사용 가능합니다."
      );
      return false;
    } else if (userID) {
      setIsIdConfirm(true);
  ...
  useEffect(() => {
    if (idValid.duplicate !== "initial" && isIdConfirm && !idValid.duplicate)
      alert("사용가능한 아이디입니다.");
    else if (
      idValid.duplicate !== "initial" &&
      isIdConfirm &&
      idValid.duplicate
    )
      alert("중복된 아이디입니다.");
        ...
        <div>
          ...
          <div className="relative">
            <div className="w-full p-2 mt-1 border border-egGrey-default">
              <div
                className={
                  residence ? "text-egBlack-default" : "text-egGrey-default"
                }
              >
