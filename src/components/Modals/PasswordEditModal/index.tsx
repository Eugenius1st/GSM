...
const PasswordEditModal = ({ activeFunc }: PasswordEditModalType) => {
  ...
  function handleEditPassword() {
    ...
    if (!pwRegex.test(pw) || !pwRegex.test(newPw)) {
      alert(
        "Password는 8~20자의 영문 대소문자, 숫자, 특수문자로 이루어져야 합니다."
      );
      ...
      {isShow ? (
        <div className="fixed flex justify-center items-center top-0 left-0 w-screen h-screen bg-[rgba(0,0,0,0.5)] border border-red-100 z-[60]">
          <div className="fixed bg-egWhite-default z-[70] w-[30rem] p-4 rounded-lg">
            ...
            <div className="my-4 text-base font-bold text-center">
              비밀번호 변경
            </div>
            <div className="flex flex-col">
              <div className="flex items-center text-egPurple-default font-semibold text-sm">
                ID
              </div>
              ...
              <input
                type="text"
                placeholder="ID"
                className="border border-egPurple-default p-1 w-full my-1"
                value={id}
                onChange={(e) => setId(e.target.value)}
              />
              <div className="flex items-center mt-2 text-egPurple-default font-semibold text-sm">
                PASSWORD
              </div>
              <input
                type="password"
            ...
            <div className="flex justify-end text-sm mt-4">
              ...
              <div>
                <PurpleBtn content={"변경"} func={handleEditPassword} />
              </div>
            </div>
          </div>
          <button
