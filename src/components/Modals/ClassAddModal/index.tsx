...
const ClassAddModal = ({ isSuccess, setIsSuccess }: ClassAddModalType) => {
  ...
  const postClass = () => {
    const coachIdArray = coaches
      ? coaches.map((coach: { _id: string }) => coach._id)
      : [];
  ...
  return (
    <div>
      <WhiteBtn content="+ 수업 추가" func={handleShowModal} />
      {isShow && isSuccess ? (
        <BasicAlert
          ...
          <div className="fixed bg-egWhite-default z-[70] w-[30rem] p-4 rounded-lg">
            ...
            <div className="p-4">
              ...
              {className === "엘리트반" && (
                <div className="flex mb-2 items">
                  <span className="w-20 mr-4 text-lg font-semibold">
                    수업분류
                  </span>
                    ...
                    <div className="flex ">
                      {coaches.map((el: any, idx: number) => (
                        <div className="flex items-center px-1 mr-1 text-sm rounded-sm bg-egBlack-superLight w-fit">
                          {el.name}{" "}
                          <CgClose onClick={() => handleDeleteCoaches(idx)} />
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div>
              <div className="text-end">
                <PurpleBtn content="추가" func={submitData} />
              </div>
            </div>
          </div>
          <button
