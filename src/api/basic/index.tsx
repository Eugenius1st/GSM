...
export async function requestGet({ requestUrl, successFunc, flagCheckFunc }: RegistGetType) {
    try {
        ...
        if (error.response) {
            alert(error.response.data.message ? error.response.data.message : '잘못된 요청입니다.');
...
export async function requestDelete({ requestUrl, data, successFunc, flagCheckFunc }: RegistDeleteType) {
    try {
        const response = await axios.delete(`${process.env.REACT_APP_API_URL}${requestUrl}`, { data: data });
